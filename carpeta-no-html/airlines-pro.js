...
for (let i=0; i < flights.length; i++){
    if(flights[i].scale == false){
        console.log(`-- Id ${flights[i].id} -- El vuelo con origen: ${flights[i].to}, y destino: ${flights[i].from} tiene un coste de ${flights[i].cost} € y no realiza ninguna escala.`) 
        }else{
            console.log(`-- Id ${flights[i].id} -- El vuelo con origen: ${flights[i].to}, y destino: ${flights[i].from} tiene un coste de ${flights[i].cost} € y realiza, al menos, una escala.`)  
...
function IsAdmin (){
    ...
    https://es.stackoverflow.com/questions/248372/como-validar-solo-numeros-en-un-prompt-en-javascript
    */
        if(isNaN(NombreUser) && NombreUser != null && NombreUser != "" && contraseña === contraseña_user){
            alert(`Se han concedido permisos de administrador.`);
            permisos_user = 'Admin';
          break;
        }else if(isNaN(NombreUser) && NombreUser != null && NombreUser != "" && contraseña !== contraseña_user && intentos < 5){
            alert('La contraseña es incorrecta, vuelva a introducir. ¡Gracias!');
            intentos = intentos + 1;
            continue;
        }else{
            alert('Has superado el numero maximo de intentos, se mantienen los permisos de usuario.');
            permisos_user = 'User';
            break;
        }
    }
...
function UserAdmin (){
    ...
    if (opcion.toLowerCase() !== 'salir'){
        ...
        UserAdBu = confirm ('Quiere seguir realizando operaciones como administrador');
    }else{
        UserAdBu = false ;
    }
...
function Añadir (){
    ...
    if(flights.length < 15){
        alert ('Para añadir un nuevo vuelo necesitamos los siguientes parametros: origen, destino, coste y escala')
        ...
        flights.push({id: flights.length, to: ori, from: des, cost: cos*1, scale: esc});
        Printflights (); 
    }else{
    ...
    switch (prec1.toLowerCase()) {
        case 'mayor':
            ...
            for(let i = 0 ; i < mayor_aux1.length ; i++ ){
                console.log(`-- Id ${mayor_aux1[i].id} -- El vuelo con origen: ${mayor_aux1[i].to}, y destino: ${mayor_aux1[i].from} tiene un coste de ${mayor_aux1[i].cost} € y realiza, al menos, una escala.`) 
            }
